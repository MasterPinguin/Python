...
def scarica_immagine_da_google(immagini_mostrate):
    ...
    # Mischia casualmente la lista delle parole chiave
    random.shuffle(parole_chiave)

    for parola_chiave in parole_chiave:
        # Cerca immagini con i parametri di ricerca specificati
        search_params = {
            'q': parola_chiave.chiave,
            'num': 1,  # Scarica solo 1 immagine alla volta
            'fileType': 'jpg',
            'safe': 'medium',
            'start': parola_chiave.n,  # Utilizza l'indice aggiuntivo per ottenere le immagini in ordine di rilevanza
        }
        ...
        try:
            ...
            if risultati:
                ...
                numero_immagine = len(immagini_mostrate) + 1
                nuova_immagine = f'/Users/newmac/Documents/PROG/Pythone/aimmagini/immagini/immagine_{numero_immagine}.jpg'
                immagine_corrente.download(nuova_immagine)
                # Incrementa il contatore di parole chiave solo se è stata trovata un'immagine
                parola_chiave.n += 1
                # Aggiorna la lista delle immagini mostrate
...
def mostra_immagine_casuale():
    ...
    immagine_corrente = scarica_immagine_da_google(immagini_mostrate)
