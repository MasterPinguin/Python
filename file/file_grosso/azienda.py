...
aggiorna_numeri('file/file_grosso/sede.txt', 'file/file_grosso/dipendente.txt', 'file/file_grosso/contratto.txt')
...
def inserimento(file):
    ...
    for i, attributo in enumerate(attributi):
        if(i==0):
            ...
            codice = -1
            fcod = 'file/file_grosso/dipendente.txt' 
            ...
            codice = -1
            fcod = 'file/file_grosso/cliente.txt' 
            while(not codice_esiste(fcod, codice)):
                try:
                    codice = int(input(f"Inserisci il nuovo valore numerico per {attributo}: "))
                except ValueError:
            ...
            codice = -1
            fcod = 'file/file_grosso/sede.txt' 
            while(not codice_esiste(fcod, codice)):
                try:
                    codice = int(input(f"Inserisci il nuovo valore numerico per {attributo}: "))
                except ValueError:
...
def clienti_per_dipendente(file):
    ...
    codici_cliente = []
    with open('file/file_grosso/contratto.txt', 'r') as f_contratti:
        righe_contratti = f_contratti.readlines()
        # Itera attraverso le righe a partire dalla seconda riga
        for riga in righe_contratti[1:]:
            valori_riga = riga.strip().split(';')
...
def visualizza(file):
    if file == 'file/file_grosso/contratto.txt' :
        ...
        for riga in nuove_righe[1:]:
            riga[1]=Nome_e_Cognome(riga[1], 'file/file_grosso/cliente.txt')
            riga[2]=Nome_e_Cognome(riga[2], 'file/file_grosso/dipendente.txt')
            print(';'.join(riga))
    elif file == 'file/file_grosso/dipendente.txt':
        print(';'.join(leggi_attributi(file)))
        nuove_righe = []
        with open(file, 'r') as f:
            righe = f.readlines()
            for riga in righe[1:]:
                nuove_righe.append(riga.strip().split(';'))
        for riga in nuove_righe[1:]:
            riga[13]=Nome_e_Cognome(riga[13], 'file/file_grosso/sede.txt')
            print(';'.join(riga))
    else:
        with open(file, "r") as f: 
            for line in f: 
...
def Nome_e_Cognome(codice, file):
    if file=='file/file_grosso/sede.txt':
        with open(file, 'r') as f:
            for line in f:
                if line.startswith(str(codice) + ';'): #se inizia con questo vero
